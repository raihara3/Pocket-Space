...
import {
  receiveMessagingHandler,
  sendMeshHandler,
} from '../../src/emitter/Messaging'
...
const Call = () => {
  ...
  const onStartWebAR = async () => {
    ...
    receiveMessagingHandler(socket, webGL, audioMedia, (list) =>
      setMemberList(list)
    )
    ...
    controller.addEventListener('selectend', () => {
      ...
      sendMeshHandler(socket, {
        ...
        vertices: JSON.stringify(
          data.geometries[0].data.attributes.position.array
        ),
  ...
  useEffect(() => {
    (async () => {
      ...
      const isSessionSupported = await navigator['xr'].isSessionSupported(
        'immersive-ar'
      )
  ...
  useEffect(() => {
    setHasError(
      !isAudioPermission || responseStatus !== 200 || !isCharLengthInRange
    )
      ...
      <Wrap>
        ...
        {expire > 0 && expire <= 6 && (
          <ErrorBox>
            <Alert variant="filled" severity="warning">
              This Room has 6 hours left to expire.
              <br />
        ...
        <Card title="Set your nickname.">
          ...
          {isSupported ? (
            ...
            <a href="https://immersiveweb.dev/">WebXR not available</a>
          )}
        </Card>
